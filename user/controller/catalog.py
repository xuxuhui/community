...
from user.serializers import CatalogSerializers
...
def list(request):
    data = request.data
    kwargs = {}
    kwargs["left_id__gt"] = 1
    if data.get("catalog_type"):
        kwargs["catalog_type"] = data["catalog_type"]
    if data.get("parent_id"):
        kwargs["parent_id"] = data["parent_id"]

    catalog_list = catalogRepository.search(**kwargs)
    count = catalogRepository.count(**kwargs)

    response_data = {}
    response_data["data"] = CatalogSerializers(catalog_list, many=True).data
    response_data["count"] = count

    return Response({"status": 200, "msg": "获取列表成功", "data": response_data})
